...
class _IndexGenerator {
  ...
  factory _IndexGenerator.build(List<String> args) {
    ...
    return new _IndexGenerator._(config);
  }


  Future run() async {
    var newPackages = await new DbPackageLoader(config).getAllGeneratedPackages(lastDate);
    if (newPackages.isNotEmpty) {
      packages.addAll(newPackages);
      _logger.info("Number of new packages - ${newPackages.length}");

      var groupedPackages = groupBy(packages, (Package p) => p.packageInfo.version).values;
      await new Generator404(config).generate();
      new HtmlIndexGenerator(config, groupedPackages)..generate();//..generatePackagePages();
